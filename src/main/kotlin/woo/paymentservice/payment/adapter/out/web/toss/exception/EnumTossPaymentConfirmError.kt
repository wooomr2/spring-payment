...
/** 토스 페이먼츠 결제 승인 에러 enum */
enum class EnumTossPaymentConfirmError(val statusCode: Int, val description: String) {
    // 400
    ...
    fun isUnknownError(): Boolean {
        return isSuccessError().not() && isFailureError().not()
    }
    ...
    companion object {
        fun get(errorCode: String): EnumTossPaymentConfirmError {
            return entries.find { it.name == errorCode } ?: UNKNOWN_PAYMENT_ERROR
        }
    }
}